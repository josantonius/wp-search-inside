...
function Hilitor2(id, tag, colors) {
    ...
    this.setRegex = function(input, rmvDiacritics) {
        input = input.replace(/\\([^u]|$)/g, "$1");
        input = input.replace(this.searchModeRegex, "").replace(/\s+/g, this.searchMode);
        input = input.replace(/^\||\|$/g, "");
        input = rmvDiacritics ? removeDiacritics(input) : addAccents(input);
        if (input) {
            var re = "(" + input + ")";
    ...
    this.apply = function(input) {
        ...
        if (this.setRegex(input, false)) {
            this.hiliteWords(targetNode);
        }
        if (this.setRegex(input, true)) {
            this.hiliteWords(targetNode);
        }
    };
    ...
    function addAccents(input) {
        ...
        retval = retval.replace(/a/ig, "([aàâäáãɐ̃]|ae)");
        retval = retval.replace(/c/ig, "[cç]");
        retval = retval.replace(/e/ig, "[eèéêëéẽ]");
        retval = retval.replace(/i/ig, "[iîïíĩ]");
        retval = retval.replace(/o/ig, "([oôöóõ]|oe)");
        retval = retval.replace(/u/ig, "[uùûüúũ]");
        retval = retval.replace(/y/ig, "[yÿ]");
        retval = retval.replace(/s/ig, "(ss|[sß])");
        return retval;
    }

    // Script to remove diacritics
    // lehel - http://lehelk.com/
